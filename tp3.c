...
int ei_main(int argc, char *argv[]) {
  ...
  for (int k = -step2; k <= step2; k += step1) {
    ...
    for (int l = -step2; l <= step2; l += 2 * step2) {
      ...
      ei_draw_line(main_window, start, end, black);
    }
  }
  ...
  start_pos.x = pos_text.x - 10;
  start_pos.y = pos_text.y - 10;
  polyline.point = start_pos;
  polyline.next = malloc(sizeof(struct ei_linked_point_t));
  polyline.next->point =
      (ei_point_t){start_pos.x + text_size.width + 20, start_pos.y};
  polyline.next->next = malloc(sizeof(struct ei_linked_point_t));
  polyline.next->next->point = (ei_point_t){
      start_pos.x + text_size.width + 20, start_pos.y + text_size.height + 20};
  polyline.next->next->next = malloc(sizeof(struct ei_linked_point_t));
  polyline.next->next->next->point =
      (ei_point_t){start_pos.x, start_pos.y + text_size.height + 20};
  polyline.next->next->next->next = malloc(sizeof(struct ei_linked_point_t));
  polyline.next->next->next->next->point = start_pos;
  polyline.next->next->next->next->next = NULL;

  ei_draw_polyline(main_window, &polyline, black);
  ...
  // free memory
  free(polyline.next->next->next->next);
  free(polyline.next->next->next);
  free(polyline.next->next);
  free(polyline.next);
